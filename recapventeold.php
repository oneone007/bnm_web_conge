...
<!-- Search Fields -->
    ...
    <!-- Fetch Data Button -->
    <button id="fetch-data-btn" class="px-6 py-3 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 transition duration-200 flex items-center justify-center space-x-2">
...
<!-- <button id="download-bccb-product-excel"
  ...
  <button id="download-bccb-product-excel" class="button">
    ...
    function updateEndDate() {
        ...
        // Remove automatic triggering - no events dispatched here
    }

    // Set end date when start date is selected (but don't trigger data fetch)
    startDateInput.addEventListener("change", updateEndDate);

    // Fetch Data Button - Manual trigger for data fetching
    document.getElementById("fetch-data-btn").addEventListener("click", () => {
        ...
        // Trigger data fetch only when button is clicked
        fetchAllData();
    });
    ...
    document.getElementById("refresh-btn").addEventListener("click", () => {
        ...
        // Only refresh data if dates are already selected
        const startDate = startDateInput.value;
        const endDate = endDateInput.value;
        ...
        if (startDate && endDate) {
            ...
            // Unified data fetching function - calls each endpoint individually
            async function fetchAllData() {
                ...
                // Fetch each section independently for better performance
                try {
            ...
            // Show loading state for all table sections
            function showAllLoaders() {
                // Total Recap loader
                const totalRecapTable = document.getElementById("recap-table");
                if (totalRecapTable) {
                    totalRecapTable.innerHTML = `<tr><td colspan="5" class="text-center p-4">Loading...</td></tr>`;
                }

                // Fournisseur loader
                const fournisseurTable = document.getElementById("recap-frnsr-table");
                if (fournisseurTable) {
                    fournisseurTable.innerHTML = `<tr><td colspan="4" class="text-center p-4">Loading...</td></tr>`;
                }

                // Product loader
                const productTable = document.getElementById("recap-prdct-table");
                if (productTable) {
                    productTable.innerHTML = `<tr><td colspan="4" class="text-center p-4">Loading...</td></tr>`;
                }

                // Zone loader
                const zoneTable = document.getElementById("recap-zone-table");
                if (zoneTable) {
                    zoneTable.innerHTML = `<tr><td colspan="4" class="text-center p-4">Loading...</td></tr>`;
                }

                // Client loader
                const clientTable = document.getElementById("recap-client-table");
                if (clientTable) {
                    clientTable.innerHTML = `<tr><td colspan="4" class="text-center p-4">Loading...</td></tr>`;
                }

                // Operator loader
                const operatorTable = document.getElementById("recap-operator-table");
                if (operatorTable) {
                    operatorTable.innerHTML = `<tr><td colspan="4" class="text-center p-4">Loading...</td></tr>`;
                }

                // BCCB loader
                const bccbTable = document.getElementById("recap-bccb-table");
                if (bccbTable) {
                    bccbTable.innerHTML = `<tr><td colspan="4" class="text-center p-4">Loading...</td></tr>`;
                }
            }
...
<tr class="dark:bg-gray-700">
    <td class="border px-4 py-2 dark:border-gray-600 text-lg font-bold text-blue-600 dark:text-blue-400">From ${startDate} to ${endDate}</td>
...
`;
            ...
            // Removed automatic date change event listeners
            // Data fetching is now only triggered by the "Fetch Data" button
    ...
    const downloadUrl = API_CONFIG.getApiUrl(`/download-totalrecap-excel?start_date=${startDate}&end_date=${endDate}&ad_org_id=1000000`);
...
let fullData = [];
    ...
    tableBody.addEventListener("click", function (event) {
        ...
        let selectedFournisseurs = [...document.querySelectorAll("#recap-frnsr-table .selected-row")]
        ...
        // Auto-trigger ALL data fetch to update ALL tables with this filter
        fetchAllData();
    });
});
...
document.getElementById("download-fournisseur").addEventListener("click", async function () {
    ...
    try {
        ...
        alert("Failed to download the Excel file.");
            ...
            // Filter inputs trigger data fetching only if dates are already selected (for cross-filtering)
            document.getElementById("recap_fournisseur").addEventListener("input", debounce(() => {
                const startDate = document.getElementById("start-date").value;
                const endDate = document.getElementById("end-date").value;
                if (startDate && endDate) fetchAllData();
            }, 500));
            
            document.getElementById("recap_product").addEventListener("input", debounce(() => {
                const startDate = document.getElementById("start-date").value;
                const endDate = document.getElementById("end-date").value;
                if (startDate && endDate) fetchAllData();
            }, 500));
            
            document.getElementById("recap_client").addEventListener("input", debounce(() => {
                const startDate = document.getElementById("start-date").value;
                const endDate = document.getElementById("end-date").value;
                if (startDate && endDate) fetchAllData();
            }, 500));
            
            document.getElementById("recap_operateur").addEventListener("input", debounce(() => {
                const startDate = document.getElementById("start-date").value;
                const endDate = document.getElementById("end-date").value;
                if (startDate && endDate) fetchAllData();
            }, 500));
            ...
            document.getElementById("recap_zone").addEventListener("input", debounce(() => {
                const startDate = document.getElementById("start-date").value;
                const endDate = document.getElementById("end-date").value;
                if (startDate && endDate) fetchAllData();
            }, 500));
...
document.getElementById("download-product-excel").addEventListener("click", async function () {
    ...
    try {
        ...
        alert("Failed to download the Excel file.");
            ...
            // Note: Event listeners are now centralized above - each section has its own optimized listeners
            ...
            document.addEventListener("DOMContentLoaded", () => {
    const zoneTableBody = document.getElementById("recap-zone-table");
    const zoneInput = document.getElementById("recap_zone");

    zoneTableBody.addEventListener("click", function (event) {
        let row = event.target.closest("tr");
        if (!row || row.id === "loading-row") return;
        ...
        // Get all selected rows
        let selectedZones = [...document.querySelectorAll("#recap-zone-table .selected-row")]
            .map(row => row.cells[0].textContent.trim());

        // Update the input field (simulate manual typing)
        zoneInput.value = selectedZones.join(", ");
...
function formatNumber(value) {
    return new Intl.NumberFormat("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value || 0);
            ...
            // Note: Zone event listeners are centralized above for better optimization
...
async function fetchClientRecap(page = 1) {
    ...
    try {
        ...
        console.error("Error fetching client recap data:", error);
            ...
            document.addEventListener("DOMContentLoaded", () => {
    const clientTableBody = document.getElementById("recap-client-table");
    const clientInput = document.getElementById("recap_client");
    ...
    clientTableBody.addEventListener("click", function (event) {
        ...
        // Get all selected rows
        let selectedClients = [...document.querySelectorAll("#recap-client-table .selected-row")]
            .map(row => row.cells[0].textContent.trim());

        // Update the input field (simulate manual typing)
        clientInput.value = selectedClients.join(", ");
...
document.getElementById("cllastPage").addEventListener("click", () => {
    ...
    fetchClientRecap(currentClientPage);
            ...
            // Note: Client event listeners are centralized above for better optimization
...
document.addEventListener("DOMContentLoaded", () => {
    const operatorTableBody = document.getElementById("recap-operator-table");
    const operatorInput = document.getElementById("recap_operateur");

    operatorTableBody.addEventListener("click", function (event) {
        let row = event.target.closest("tr");
        if (!row || row.id === "loading-row") return;
        ...
        // Get all selected rows
        let selectedOperators = [...document.querySelectorAll("#recap-operator-table .selected-row")]
            .map(row => row.cells[0].textContent.trim());

        // Update the input field (simulate manual typing)
        operatorInput.value = selectedOperators.join(", ");
...
function formatNumber(num) {
    return new Intl.NumberFormat().format(num);
            ...
            // Note: Operator event listeners are centralized above for better optimization
...
function changeBccbPage(page) {
    ...
    // Trigger all data fetch for cross-filtering
    fetchAllData();
    ...
    if (!bccbInput) {
        ...
        document.getElementById("bccb-product-container").style.display = "none";
            ...
            document.addEventListener("DOMContentLoaded", () => {
    const bccbTableBody = document.getElementById("recap-bccb-table");
    const bccbInput = document.getElementById("recap_bccbclient");

    bccbTableBody.addEventListener("click", function (event) {
        let row = event.target.closest("tr");
        if (!row || row.id === "loading-row") return;
        ...
        // Update input field
        bccbInput.value = selectedBccb;

        // Manually trigger input event
        bccbInput.dispatchEvent(new Event("input"));

        // Auto-trigger ALL data fetch to update ALL tables with this filter
        fetchAllData();

        // Fetch BCCB Product (Additional specific functionality for BCCB)
        fetchBccbProduct(selectedBccb);
    });
});

          
            // Note: BCCB event listeners are centralized above for better optimization
...
recapInputs.forEach(id => {
    ...
    if (input) {
        input.addEventListener('focus', function() {
            ...
            // Reset countdown when user interacts with filters
            ['start-date', 'end-date', 'recap_fournisseur', 'recap_product', 'recap_zone', 'recap_client', 'recap_operateur', 'recap_bccbclient'].forEach(id => {
                const element = document.getElementById(id);
                if (element) {
                    element.addEventListener('input', () => {
                        countdownSeconds = 30; // Reset countdown on interaction
                    });
                }
            });
