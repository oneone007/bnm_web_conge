...
var loadingAnimation = lottie.loadAnimation({
    ...
    async function fetchRemiseData() {
        try {
            const response = await fetch('http://192.168.1.156:5000/fetch-remise-data');
...
document.getElementById("downloadExcel").addEventListener("click", function () {
    ...
    let url = `http://192.168.1.156:5000/download-marge-excel?fournisseur=${encodeURIComponent(fournisseur)}&product=${encodeURIComponent(product)}&marge=${encodeURIComponent(marge)}`;
    window.open(url, "_blank");
});

document.getElementById("downloadExcel_REMISE").addEventListener("click", function () {
    window.open("http://192.168.1.156:5000/download-remise-excel", "_blank"); 
});
document.getElementById("downloadExcel_BONUS").addEventListener("click", function () {
    window.open("http://192.168.1.156:5000/download-bonus-excel", "_blank"); 
});
document.getElementById("downloadExcel_RESERVE").addEventListener("click", function () {
    window.open("http://192.168.1.156:5000/download-reserved-excel", "_blank"); 
});


async function fetchData() {
    try {
        const response = await fetch('http://192.168.1.156:5000/fetch-data');
        if (!response.ok) throw new Error('Network response was not ok');
...
async function fetchBonusData() {
    try {
        const response = await fetch('http://192.168.1.156:5000/fetch-bonus-data');
        if (!response.ok) throw new Error('Network response was not ok');
...
async function fetchReservedData() {
    try {
        const response = await fetch('http://192.168.1.156:5000/fetch-reserved-data');
        if (!response.ok) throw new Error('Network response was not ok');
